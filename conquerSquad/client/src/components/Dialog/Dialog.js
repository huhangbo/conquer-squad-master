...
export function alert (content){
    const component = <Dialog visible={true} title={"提示"} onOk={true} children={content}
                              handleOk={() => {clearElement(component,div)}}/>
    const div = renderElement(component)
}

export function confirm (content, ok=()=>{}, cancel=()=>{}) {
    const component = <Dialog visible={true} title={"确认"} onOk={true} onCancle={true} children={content}
                              handleOk={() => {
                                  ok()
